...
/** MiniCluster-based integration test for the {@link OwnFileSource}. */
class FileSourceTextLinesITCase {
  ...
  @Test
  void testOwnBoundedTextFileSourceThrowsException(
      @TempDir java.nio.file.Path tmpTestDir, @InjectMiniCluster MiniCluster miniCluster) {
    Exception exception =
        assertThrows(
            org.apache.flink.runtime.client.JobExecutionException.class,
            () -> {
              testOwnBoundedTextFileSource(tmpTestDir, false);
            });

    // String message = exception.getCause().getCause().getCause().getMessage();

    Throwable jobException = exception.getCause();
    assertNotNull(jobException);
    Throwable flinkException = jobException.getCause();
    assertNotNull(flinkException);
    Throwable unsupportedOperationException = flinkException.getCause();
    assertNotNull(unsupportedOperationException);

    assertEquals(UnsupportedOperationException.class, unsupportedOperationException.getClass());

    String expectedMessage =
        "The bounded case is not supported here. If you want to use a bounded file source, use flink's build in file source!";
    String actualMessage = unsupportedOperationException.getMessage();

    assertTrue(actualMessage.contains(expectedMessage));
  }

  @Test
  void testOwnBoundedTextFileSourceThrowsExceptionWithBatchMode(
      @TempDir java.nio.file.Path tmpTestDir, @InjectMiniCluster MiniCluster miniCluster) {
    Exception exception =
        assertThrows(
            org.apache.flink.runtime.client.JobExecutionException.class,
            () -> {
              testOwnBoundedTextFileSource(tmpTestDir, true);
            });
  ...
  private void testOwnBoundedTextFileSource(java.nio.file.Path tmpTestDir, boolean batchMode)
      throws Exception {
    ...
    final OwnFileSource<String> source =
        OwnFileSource.forRecordStreamFormat(new TextLineInputFormat(), Path.fromLocalFile(testDir))
            .build();
    ...
    env.execute("Bounded TextFiles Test");
  }

  /**
   * This test runs a job reading continuous input (files appearing over time) with a stream record
   * format (text lines).
   */
  @Test
  ...
  /**
   * This test runs a job reading continuous input (files appearing over time) with a stream record
   * format (text lines) and restarts TaskManager.
   */
  @Test
  void testContinuousTextFileSourceWithTaskManagerFailover(@TempDir java.nio.file.Path tmpTestDir)
    ...
    runTestWithNewMiniCluster(
        miniCluster -> testContinuousTextFileSource(tmpTestDir, FailoverType.TM, miniCluster));
  }

  /**
   * This test runs a job reading continuous input (files appearing over time) with a stream record
   * format (text lines) and triggers JobManager failover.
   */
  @Test
  void testContinuousTextFileSourceWithJobManagerFailover(@TempDir java.nio.file.Path tmpTestDir)
    ...
    runTestWithNewMiniCluster(
        miniCluster -> testContinuousTextFileSource(tmpTestDir, FailoverType.JM, miniCluster));
  }

  private void testContinuousTextFileSource(
      java.nio.file.Path tmpTestDir, FailoverType type, MiniCluster miniCluster) throws Exception {
    final File testDir = tmpTestDir.toFile();
  ...
  private static void writeFileAtomically(
      final File file,
      final String[] lines,
      final FunctionWithException<OutputStream, OutputStream, IOException> streamEncoderFactory)
      throws IOException {
