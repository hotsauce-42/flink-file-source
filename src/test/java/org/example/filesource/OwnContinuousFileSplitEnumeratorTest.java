...
import org.example.filesource.testutils.TestingFileEnumerator;
import org.example.filesource.testutils.TestingSplitEnumeratorContext;
import org.junit.jupiter.api.Test;

/** Unit tests for the {@link OwnContinuousFileSplitEnumerator}. */
class OwnOwnContinuousFileSplitEnumeratorTest {
  ...
  void testDiscoverSplitWhenNoReaderRegistered() throws Exception {
    ...
    final OwnContinuousFileSplitEnumerator enumerator = createEnumerator(fileEnumerator, context);
  ...
  private static FileSourceSplit createRandomSplit() {
    return new FileSourceSplit(
        String.valueOf(splitId++), Path.fromLocalFile(new File(TMP_DIR, "foo")), 0L, 0L, 0L, 0L);
  }

  private static OwnContinuousFileSplitEnumerator createEnumerator(
      final FileEnumerator fileEnumerator, final SplitEnumeratorContext<FileSourceSplit> context) {

    final OwnContinuousFileSplitEnumerator enumerator =
