...
/**
 * Adapter that turns a {@link StreamFormat} into a {@link BulkFormat}, opening the file through a
 * {@link FileSystem} built from {@link S3FileSystemSettings} instead of the globally initialised
 * one.
 *
 * <p>This is a fork of Flink's {@code StreamFormatAdapter}; the only behavioural change is that
 * {@link #openStream} resolves the {@code FileSystem} from the per-source settings rather than from
 * {@code path.getFileSystem()}. Compression-by-extension was dropped, as it is not needed for the
 * object-store use case.
 */
@Internal
public final class MinioStreamFormatAdapter<T> implements BulkFormat<T, FileSourceSplit> {

  private static final long serialVersionUID = 1L;

  private static final Logger LOG = LoggerFactory.getLogger(MinioStreamFormatAdapter.class);

  private final StreamFormat<T> streamFormat;
  private final S3FileSystemSettings settings;

  public MinioStreamFormatAdapter(StreamFormat<T> streamFormat, S3FileSystemSettings settings) {
    this.streamFormat = checkNotNull(streamFormat);
    this.settings = checkNotNull(settings);
  }

  @Override
  public BulkFormat.Reader<T> createReader(final Configuration config, final FileSourceSplit split)
      throws IOException {
  ...
  @Override
  public BulkFormat.Reader<T> restoreReader(final Configuration config, final FileSourceSplit split)
      throws IOException {
    ...
    return doWithCleanupOnException(
        ...
        () -> {
          ...
          // reader like a fresh reader. See the JavaDocs on StreamFormat.restoreReader().
          final StreamFormat.Reader<T> streamReader =
  ...
  private TrackingFsDataInputStream openStream(
      final Path file, final Configuration config, final long seekPosition) throws IOException {
  ...
  public static final class Reader<T> implements BulkFormat.Reader<T> {
    ...
    public RecordIterator<T> readBatch() throws IOException {
      ...
      final RecordIterator<T> iter =
          new IteratorResultIterator<>(result.iterator(), lastOffset, lastRecordsAfterOffset);
   ...
   * should finish the current batch and start the next batch, making batch sizes depend on the
   * consumed data volume rather than on a record count.
   */
